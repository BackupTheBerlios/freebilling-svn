...
void billingreports::newDoc()
{
    billingreports *ed = new billingreports;
    ed->setCaption(tr("Qt Example - Application"));
    ed->show();
}

void billingreports::choose()
{
...
void billingreports::load( const QString &fileName )
{
    QFile f( fileName );
...
void billingreports::closeEvent( QCloseEvent* ce )
{
    if ( !e->isModified() ) {
	ce->accept();
	return;
